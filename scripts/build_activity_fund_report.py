...
def flow_matrix(src, signed=False):
    """district x year matrix of a $ flow (revenue / expenditure / net). Each cell renders BOTH a
    total ($thousands) and a per-student value as two spans (.vt / .vp); a pure-CSS radio toggle
    shows one or the other (no JavaScript). signed=True colors negatives red / positives green."""
    def fmt(v, perstu):
        if perstu:
    ...
    def cell(v, e, extra=""):
        t_tot, t_ps = fmt(v, False), (fmt(v / e, True) if e else "—")
        return (f'<td class="fcell{extra}"><span class="vt">{t_tot}</span>'
                f'<span class="vp">{t_ps}</span></td>')
    ...
    for d in DISTS:
        ...
        for y in YEARS:
            ...
            tds.append(cell(v, e, extra=f'" style="{style}'))
        out.append(f"<tr>{''.join(tds)}</tr>")
    # peer-average row (average of per-district totals; average of per-student values)
    avg_tds = ['<th class="dname avg">Peer average (excl. ICCSD)</th>']
    for y in YEARS:
        ...
        avg_tds.append(f'<td class="fcell avg"><span class="vt">{a_tot}</span>'
                       f'<span class="vp">{a_ps}</span></td>')
    out.append(f'<tr class="avgrow">{"".join(avg_tds)}</tr>')
    return "\n".join(out)
...
td.na{{color:#cbd5e1}}
.flowradio{{position:absolute;width:1px;height:1px;opacity:0;pointer-events:none}}
...
.tg{{font:600 13px/1 inherit;color:#2563eb;background:#fff;border:1px solid #cbd5e1;padding:6px 13px;border-radius:999px;cursor:pointer;user-select:none}}
.tg:hover{{background:#eff6ff;border-color:#bfdbfe}}
/* pure-CSS toggle: the checked radio (sibling of .wrap) drives which span shows + the active pill */
.vp{{display:none}}
#fm-ps:checked ~ .wrap .vt{{display:none}}
#fm-ps:checked ~ .wrap .vp{{display:inline}}
#fm-total:checked ~ .wrap label.tg[for="fm-total"],
#fm-ps:checked ~ .wrap label.tg[for="fm-ps"]{{background:#2563eb;color:#fff;border-color:#2563eb}}
.flowradio:focus-visible ~ .wrap label.tg{{outline:2px solid #2563eb;outline-offset:2px}}
...
<div class="toggle" role="group" aria-label="Show values as">
  ...
  <label class="tg" for="fm-total">Total $</label>
  <label class="tg" for="fm-ps">Per student</label>
</div>
