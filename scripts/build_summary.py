#!/usr/bin/env python3
"""One-screen summary table: Iowa City vs. all large peers vs. best-run peers, with targets.
...
# (label, getter, fmt, dir, good, ok, target_text)
ROWS = [
 ("Overall financial score (of 5)", lambda c:c["composite"], lambda v:f"{v:.1f}", "hi", 4, 3, "≥ 4.0"),
 ("Spending authority left — UAB (%)", lambda c:c["uab_last"], lambda v:f"{v:.0f}%", "hi", 10, 5, "Positive; ≥10%"),
 ("Rainy-day reserves / solvency (%)", lambda c:c["solv_last"], lambda v:f"{v:.0f}%", "hi", 10, 5, "5–15%"),
 ("Days of operating reserves", lambda c:c.get("days_reserves"), lambda v:f"{v:.0f}", "hi", 60, 30, "≥ 60 (2 mo.)"),
 ("Operating margin, 3-yr avg (%)", lambda c:c["marg3"], lambda v:f"{v:+.1f}%", "hi", 0, -2, "≥ 0%"),
 ("Most recent audit (yrs behind)", yrs_behind, lambda v:("current" if v<=0 else f"{v:.0f} yr"+("s" if v>=2 else "")), "lo", 0, 1, "Current (≤180 d)"),
 ("Financial-management quality (of 5)", lambda c:c["quality"], lambda v:f"{v:.1f}", "hi", 4, 3, "≥ 4.0"),
 ("Cash-reserve tax used (% of cap)", lambda c:c["crl_pct"], lambda v:f"{v:.0f}%", "lo", 25, 40, "— (lower)"),
 ("School property-tax rate ($/$1,000)", lastlevy, lambda v:f"${v:.2f}", "lo", 14.5, 16.5, "— (lower)"),
 ("Building debt per student", debt_pp, lambda v:f"${v/1000:.0f}K", "lo", 12000, 20000, "— (< 5% limit)"),
 ("Annual debt payments per student", annual_pp, lambda v:f"${v:,.0f}", "lo", 1500, 2500, "— (lower)"),
 ("Years of SAVE revenue pledged", lambda c:c.get("save_years"), lambda v:f"{v:.1f}", "lo", 5, 7, "≤ ~10–15 yrs"),
 ("Enrollment trend (%/yr)", lambda c:c["enr_cagr"], lambda v:f"{v:+.1f}%", "ctx", 0, 0, "— (context)"),
]
def cls(v, d, good, ok):
...
rows_html=""
for label, get, f, d, good, ok, target in ROWS:
...
th,td{{padding:9px 11px;text-align:center;border-bottom:1px solid #eef2f7}}
th{{background:#1f4e79;color:#fff;font-size:11.5px;text-transform:uppercase;letter-spacing:.03em}}
...
td.tgt{{color:#475569;font-size:12.5px;white-space:nowrap;background:#f8fafc}} th.tgt{{background:#2c5d8f}}
...
.foot{{color:#94a3b8;font-size:11.5px;margin-top:12px;line-height:1.5}}
...
<table>
<thead><tr><th>Measure</th><th class="tgt">Healthy target</th><th class="ic">Iowa City</th><th>All large peers (avg)</th><th>Best-run peers (avg)</th></tr></thead>
...
<p class="foot">Targets cite published standards where one exists — GFOA (≥60 days / ~2 months of reserves; audits filed timely), and ISFIS/IASB Iowa guidance (solvency 5–15%). "—" means there is no single accepted target; the arrow shows which direction is better. SAVE "~10–15 yrs" is the district's own historical bonding practice. Every figure traces to a district's audited financial report or an official Iowa state filing.</p>
</div></body></html>"""
open("iccsd-summary.html","w").write(html)
print(f"Wrote iccsd-summary.html ({len(html)//1024} KB), {len(ROWS)} rows, with target column")
